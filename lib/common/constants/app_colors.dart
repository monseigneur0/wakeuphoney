import 'package:flutter/material.dart';

class AppColors {
  static const Color veryDarkGrey = Color.fromARGB(255, 18, 18, 18);
  static const Color darkGrey = Color.fromARGB(255, 45, 45, 45);
  static const Color grey = Color.fromARGB(255, 139, 139, 139);
  static const Color middleGrey = Color.fromARGB(255, 171, 171, 171);
  static const Color brightGrey = Color.fromARGB(255, 228, 228, 228);
  static const Color blueGreen = Color.fromARGB(255, 0, 185, 206);
  static const Color green = Color.fromARGB(255, 132, 206, 191);
  static const Color darkGreen = Color.fromARGB(255, 101, 160, 149);
  static const Color blue = Color.fromARGB(255, 0, 125, 203);
  static const Color darkBlue = Color.fromARGB(255, 0, 70, 111);
  static const Color mediumBlue = Color.fromARGB(255, 60, 140, 180);
  static const Color darkOrange = Color.fromARGB(255, 222, 112, 48);
  static const Color faleBlue = Color.fromARGB(255, 160, 206, 222);
  static const Color brightBlue = Color.fromARGB(255, 123, 182, 212);
  static const Color salmon = Color(0xffff6666);

  static const Color myPink = Color(0xFFD72499);
  static const Color myBackgroundPink = Color.fromARGB(221, 255, 250, 250);
  static const Color myAppBarBackgroundPink = Color(0xDDFDE7F6);
  static const Color sleepingbear = Color(0xFFFEBE5D);
  static const Color awakebear = Color(0xFFFEE7CD);
  static const Color rabbitspeak = Color(0xFFFDE8CB);
  static const Color rabbitwake = Color(0xFFFCFAF9);
  static const Color rabbitalarm = Color(0xFFFFCFA1);

  static const Color seedColor = Color(0xFF120338);
  static const Color primary = Color(0xFF120338);
  static const Color primary900 = Color(0xFF120338);
  static const Color primary800 = Color(0xFF22075E);
  static const Color primary700 = Color(0xFF391085);
  static const Color primary600 = Color(0xFF531DAB);
  static const Color primary500 = Color(0xFF722ED1);
  static const Color primary400 = Color(0xFF9254DE);
  static const Color primary300 = Color(0xFFB37FEB);
  static const Color primary200 = Color(0xFFD3ADF7);
  static const Color primary100 = Color(0xFFEFDBFF);
  static const Color primary50 = Color(0xFFF9F0FF);

  static const Color point900 = Color(0xFFA67DE0);
  static const Color point800 = Color(0xFFB29ECD);
  static const Color point700 = Color(0xFFDED6EC);
  static const Color point600 = Color(0xFFF5F2FA);

  static const Color grey900 = Color(0xFF222222);
  static const Color grey800 = Color(0xFF424242);
  static const Color grey700 = Color(0xFF636666);
  static const Color grey600 = Color(0xFF7C7C7C);
  static const Color grey500 = Color(0xFF9E9E9E);
  static const Color grey400 = Color(0xFFD0D0D0);
  static const Color grey300 = Color(0xFFE3E3E3);
  static const Color grey200 = Color(0xFFEEEEEE);
  static const Color grey100 = Color(0xFFF6F6F6);

  static const Color rgbSeedColor = Color.fromARGB(255, 18, 3, 56);
  static const Color rgbPrimary = Color.fromARGB(255, 18, 3, 56);
  static const Color rgbPrimary900 = Color.fromARGB(255, 18, 3, 56);
  static const Color rgbPrimary800 = Color.fromARGB(255, 34, 7, 94);
  static const Color rgbPrimary700 = Color.fromARGB(255, 57, 16, 133);
  static const Color rgbPrimary600 = Color.fromARGB(255, 83, 29, 171);
  static const Color rgbPrimary500 = Color.fromARGB(255, 114, 46, 209);
  static const Color rgbPrimary400 = Color.fromARGB(255, 146, 84, 222);
  static const Color rgbPrimary300 = Color.fromARGB(255, 179, 127, 235);
  static const Color rgbPrimary200 = Color.fromARGB(255, 211, 173, 247);
  static const Color rgbPrimary100 = Color.fromARGB(255, 239, 219, 255);
  static const Color rgbPrimary50 = Color.fromARGB(255, 249, 240, 255);

  static const Color rgbPoint900 = Color.fromARGB(255, 166, 125, 224);
  static const Color rgbPoint800 = Color.fromARGB(255, 178, 158, 205);
  static const Color rgbPoint700 = Color.fromARGB(255, 222, 214, 236);
  static const Color rgbPoint600 = Color.fromARGB(255, 245, 242, 250);

  static const Color rgbGrey900 = Color.fromARGB(255, 34, 34, 34);
  static const Color rgbGrey800 = Color.fromARGB(255, 66, 66, 66);
  static const Color rgbGrey700 = Color.fromARGB(255, 99, 102, 102);
  static const Color rgbGrey600 = Color.fromARGB(255, 124, 124, 124);
  static const Color rgbGrey500 = Color.fromARGB(255, 158, 158, 158);
  static const Color rgbGrey400 = Color.fromARGB(255, 208, 208, 208);
  static const Color rgbGrey300 = Color.fromARGB(255, 227, 227, 227);
  static const Color rgbGrey200 = Color.fromARGB(255, 238, 238, 238);
  static const Color rgbGrey100 = Color.fromARGB(255, 246, 246, 246);
  static const Color rgbGrey50 = Color.fromARGB(255, 250, 250, 250);

  static const Color white = Color.fromARGB(255, 236, 236, 236);
  static const Color black = Color.fromARGB(255, 0, 0, 0);
  static const Color whiteBackground = Color.fromARGB(255, 255, 255, 255);
  static const Color borderColor = Color.fromARGB(255, 171, 171, 171);
  static const Color myBackground = Color(0xFFFAFAFA);
}
