...
import 'package:wakeuphoney/features/oldmatch/match_repo.dart';
...
final checkMatchProcessProvider = StreamProvider.family((ref, int honeyCode) {
  return ref.watch(matchConrollerProvider.notifier).checkMatchProcess(honeyCode);
});

final matchConrollerProvider = StateNotifierProvider<MatchController, bool>((ref) {
...
class MatchController extends StateNotifier<bool> {
  ...
  Future<MatchModel> getOrCreateMatchCode() async {
    ...
    MatchModel? currentCode = await _matchRepository.getMatchCodeFuture(uid, inthoneycode);
    if (currentCode == null) {
      currentCode = MatchModel(uid: uid, time: DateTime.now(), vertifynumber: inthoneycode);
  ...
  Stream<MatchModel> checkMatchProcess(int honeyCode) async* {
    try {
      ...
      matched.asyncMap((event) => _ref.watch(coupleIdProvider.notifier).state = event.uid);
  ...
  void matchCoupleIdProcessDone(String coupleId) async {
    ...
    await _matchRepository.getUser(coupleId).then((value) async {
      ...
      await _matchRepository.matchCoupleIdProcessDone(
          uid, auser!.displayName ?? "", auser.photoURL ?? "", coupleId, value.displayName, value.photoURL);

      await _matchRepository.addFriend(
          uid, auser.displayName ?? "", auser.photoURL ?? "", coupleId, value.displayName, value.photoURL);
    });
  }
}
