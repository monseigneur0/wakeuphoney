...
import 'package:wakeuphoney/tabs/manager/manager_repository.dart';
...
final futureMatchNumberProvider = FutureProvider<MatchModel>((ref) {
  return ref.watch(managerControllerProvider.notifier).getFutureMatchNumber();
});

final futureRecentUserProvider = FutureProvider<List<UserModel>>((ref) {
  return ref.watch(managerControllerProvider.notifier).getRecentUser();
});

final managerControllerProvider = AsyncNotifierProvider<ManagerController, void>(() => ManagerController());

class ManagerController extends AsyncNotifier<void> {
  late final ManagerRepsoitory _repository;
  ...
  @override
  FutureOr<void> build() {
    _repository = ref.read(managerRepositoryProvider);
  }

  void createNewUser() {
    logger.d("createNewUser");
    // _repository.createNewUser();
  }

  //최근 1주일 이내 생성한 커플있는 커플 자료
  Future<List<UserModel>> getRecentUser() async {
    return await _repository.getRecentUser();
  }
